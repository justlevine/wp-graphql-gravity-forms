 ...
 * Class - CreateEntry
 */
class CreateEntry implements Hookable, Mutation {
	/**
	 * Mutation name.
	 */
	const NAME = 'createGravityFormsEntry';
	...
	public static function get_input_fields() : array {
		return [
			...
			'entryId'    => [
				'type'        => 'Integer',
				'description' => __( 'Optional. ID of the entry to update. Only used if `entryType` is `EXISTING`. ', 'wp-graphql-gravity-forms' ),
			],
			'pageNumber' => [
				'type'        => 'Integer',
	...
	public function mutate_and_get_payload() : callable {
		return function( $input, AppContext $context, ResolveInfo $info ) : array {
			...
			if ( ! isset( $input['entryType'] ) ) {
				$input['entryType'] = 'NEW';
			}

			if ( 'EXISTING' === $input['entryType'] && ! isset( $input['entryId'] ) ) {
				throw new UserError( __( 'Mutation not processed. To update an existing entry, you must include an `entryId`.', 'wp-graphql-gravity-forms' ) );
			}
	...
	private function save_draft_submission( array $input, array $form, string $source_url ) : string {
		...
		$entry          = 'EXISTING' === $input['entryType'] ? $this->get_existing_entry_data( $input['entryId'] ) : $this->get_draft_entry_data( $form, $ip, $source_url );
		$field_values   = '';
